...
public final class QueryUtils {
    ...
    private static List<Event> extractFeatureFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            // For each earthquake in the earthquakeArray, create an {@link Event} object
            for (int i = 0; i < earthquakeArray.length(); i++) {

                // Get a single event at position i within the list of events
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);

                // For a given event, extract the JSONObject associated with the
                // key called "properties", which represents a list of all properties
                // for that event.
                JSONArray dates = currentEarthquake.getJSONArray("dates");
                String description = currentEarthquake.getString("description").replaceAll("^\\s+|\\s+$", "");
                String title = currentEarthquake.getString("title");
                String location = currentEarthquake.getString("location");



                // Extract the value for the key called "time"

                String startTime = dates.getJSONObject(0).optString("starts_at");
                String endTime = dates.getJSONObject(0).getString("ends_at");



                // Extract the value for the key called "url"
                String url = dates.getJSONObject(0).getJSONObject("links").getString("web");

                String image_url = null;
                try {
                    image_url = currentEarthquake.getString("thumbnail_url");
                }
                ...
                // Create a new {@link Event} object with the magnitude, location, time,
                // and url from the JSON response.
                Event event = new Event(title, location, description, startTime, endTime, url, image_url);
